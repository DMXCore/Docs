...
| Piece | Where | State |
|-------|-------|-------|
| Docs copilot (Phase 1) | Docs widget + [HelpApi](https://github.com/DMXCore/HelpApi) at `helpapi.dmxcore.com` | **Live** on docs.dmxcore.com: BM25 retrieval, in-chat checklists, screenshots, 👍/👎, redacted transcripts (1 year), validation of docs URLs, screenshot ids, UI names and menu paths.<br>**Cost:** $200/month ceiling enforced in HelpApi, a persisted daily turn limit, and budget alerts at $100 / $200.<br>**Continue with my device** links to the portal (`HELP_CONTINUE_URL`); a chat continued there shows "continued in the portal".<br>Navigation lookup (`find_screen`) and recipe evals come from the quality workstream |
| Config snapshot builder | Core [#128](https://github.com/DMXCore/DmxCore100-Software/issues/128) (`1f087698`; timeline file names fixed in `e3980b5d`) | Done |
| Snapshot relay (`SnapshotNow`, `CONFIGSNAPSHOT`, `device-snapshots`) | DeviceApi (`fcfc18f`, deployed) | Done: unsafe entries are skipped, rejected captures are reported as `lastRejected`, trigger and TUS tests exist. The container is still created at startup, not in bicep |
| Grants, handoff, snapshot and logs endpoints, session context | AdminPortal [#28](https://github.com/DMXCore/AdminPortal/issues/28) (`867b517`, deployed) | Done: scoped help-agent tokens, context endpoints for HelpApi, session lifecycle fixes, 409 `update_required` for units older than 2026.914.1 |
| Navigation document | Core [#129](https://github.com/DMXCore/DmxCore100-Software/issues/129) (`96c5e43d`) | Done in Core; the copilot index and `find_screen` use it (quality workstream). Publishing it per release from DevOps is still open |
| User activity events in Seq | Core [#130](https://github.com/DMXCore/DmxCore100-Software/issues/130) (`72917354`) + AdminPortal (`c60c021`) | Done. Release touchscreen hosts covered by `HostLoggingTests`. `Service = 'DmxCore100'` is set by Seq on the ingestion key |
| Signed-in chat with device tools | HelpApi (`1293ab7`, `c18e4de`) + portal `/help` (`a64dcef`) | **Deployed:**<br>- the portal mints a scoped help-agent token<br>- HelpApi reads the session context each turn and binds the session to the portal user<br>- `list_sections` / `get_section`, and `get_recent_logs` when the logs grant is on<br>- the chat view in the portal<br>Not yet exercised end to end with a real device |
