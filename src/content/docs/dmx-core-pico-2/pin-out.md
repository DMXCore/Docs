...
J3 is the ARM Serial Wire Debug (SWD) port on the RP2040. It uses the Raspberry Pi 3-pin debug connector: a keyed **1.0 mm JST SH** socket (`BM03B-SRSS-TB`), the same connector as a Raspberry Pi Pico H.
...
J3 does not include a target-voltage (VTref) pin. For a J-Link, wire SWCLK, SWDIO, and GND as above, and take **3.3 V from the expansion header** into the J-Link VTref pin. There is no reset pin on J3; SWD attach without reset is enough.
