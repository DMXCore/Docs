...
* Create a plain text file at the root named `dmxcore.txt` with a single line:\
  `force-dhcp`
* Save the file and eject the USB stick
* Insert into the DMX Core 100 and reboot
* After a few minutes it should change the network settings back to DHCP. There may not be a visible indication that it has taken place, but you can contact our technical support to verify that the device has reconnected to the deployment platform.
