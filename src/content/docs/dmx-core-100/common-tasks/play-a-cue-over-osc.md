...
- Open **Control & Integrations > OSC Clients** in the Web UI. The **Recent OSC Senders** section at the bottom lists every IP that has sent OSC to the device, with the message count and the last address received, and refreshes on its own while the page is open. If your sender's IP is not listed, check the IP address, the port, and that the sender is using UDP.
