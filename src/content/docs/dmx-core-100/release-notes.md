...
| v2026.916.x | 9/16/2026 | Fixed: a [Digital Input trigger](/dmx-core-100/scheduling-automation/input-triggers#input-types) on the DMX-512 board only ever saw one edge, so a contact closure could not release a Flash preset or a Momentary timeline, and a trigger with Threshold 0 never ran anything. It now fires when the input goes active and releases when it goes inactive; Threshold 0 inverts the polarity |
