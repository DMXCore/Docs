...
| v2026.916.x | 9/16/2026 | A [Control Value](/dmx-core-100/integrations/control-values#operations) **Set value** action now offers press mode **Flash (hold)**: the value is set while the key, pad, OSC button or contact is held and what was there before comes back on release (a Toggle goes back off), so a push button on a digital input holds a Control Value On only while pressed, with one trigger instead of two |
